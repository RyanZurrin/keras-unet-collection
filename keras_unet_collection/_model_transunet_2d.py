...
def ViT_MLP(X, filter_num, activation='GELU', name='MLP'):
    ...
    for i, f in enumerate(filter_num):
        X = Dense(f, name=f'{name}_dense_{i}')(X)
...
def ViT_block(V, num_heads, key_dim, filter_num_MLP, activation='GELU', name='ViT'):
    ...
    V_atten = V # <--- skip
    V_atten = LayerNormalization(name=f'{name}_layer_norm_1')(V_atten)
    V_atten = MultiHeadAttention(
        num_heads=num_heads, key_dim=key_dim, name=f'{name}_atten'
    )(V_atten, V_atten)
    # Skip connection
    V_add = add([V_atten, V], name=f'{name}_skip_1')

    # MLP
    V_MLP = V_add # <--- skip
    V_MLP = LayerNormalization(name=f'{name}_layer_norm_2')(V_MLP)
    V_MLP = ViT_MLP(V_MLP, filter_num_MLP, activation, name=f'{name}_mlp')
    return add([V_MLP, V_add], name=f'{name}_skip_2')
